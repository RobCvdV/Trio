        ...
        return await ctx.perform {
            (result as? [OrefDetermination] ?? []).compactMap { d -> AdvisorExport.DeterminationPoint? in
                ...
                return AdvisorExport.DeterminationPoint(
                    ...
                    cob: Double(d.cob),
                    bg: d.glucose?.doubleValue.map { $0.asMmolFromMgdL },
                    isf: d.insulinSensitivity?.doubleValue.map { $0.asMmolFromMgdL },
                    target: d.currentTarget?.doubleValue.map { $0.asMmolFromMgdL },
